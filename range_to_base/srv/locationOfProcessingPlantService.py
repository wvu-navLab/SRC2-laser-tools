...
from range_to_base.srv import LocationOfProcessingPlant, LocationOfProcessingPlantResponse
...
x=np.empty(0)
y=np.empty(0)
pub = rospy.Publisher("processing_plant_location", Point, queue_size=10)
...
def range(mess):
    ...
    print("In the Location of Processing Plant Service")
	...
	# check for 'goodness of fit' by looking at size of radius, residuals, x, y
        if residu_2<1000 and (R_avg < 3.0) and (R_avg > 0.5):
...
rospy.init_node('location_of_processing_plant')

service=rospy.Service('location_of_processing_plant_service',LocationOfProcessingPlant,range)
