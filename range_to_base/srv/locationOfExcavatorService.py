...
from range_to_base.srv import LocationOfExcavator, LocationOfExcavatorResponse
...
x=np.empty(0)
y=np.empty(0)
pub = rospy.Publisher("excavtor_location", Point, queue_size=10)
...
def range(mess):
    ...
    print("In the Location of Excavator Service")

    # call the point cloud
    rospy.wait_for_service("scan_to_cloud")
    clouder = rospy.ServiceProxy("scan_to_cloud", ScanToPointCloud2);
    resp_cloud = clouder(mess.center-mess.offset,mess.center+mess.offset,0,0,1,5, 1, 1);
    ...
    if( not resp_cloud_filtered.success):
            ...
            rospy.wait_for_service("scan_to_cloud")
            clouder = rospy.ServiceProxy("scan_to_cloud", ScanToPointCloud2);
            resp_cloud = clouder(mess.center-mess.offset-0.05,mess.center+mess.offset-0.05,0,0,1,5, 1, 1);
...
rospy.init_node('location_of_excavator')

service=rospy.Service('location_of_excavator_service',LocationOfExcavator,range)
